...
type RoleName = 'AppConfigurationDataOwner' | 'AppConfigurationDataReader'
...
resource Authorization_roleAssignments_ 'Microsoft.Authorization/roleAssignments@2022-04-01' = [for authorization in authorizationList: {
  ...
  name: guid(subscription().id, AppConfiguration_configurationStores_.id, roleId[authorization.role], authorization.principal.id)
  properties: {
